...
traffic-density-yolo-kf/
│
├── detections/                # YOLOv8 per-frame detections (CSV)
├── tracks/                    # Kalman filter tracking outputs
├── results/                   # Processed outputs (counts, congestion)
│   ├── vehicle_count.csv
│   ├── congestion_score.csv
│
├── figures/                   # Generated figures for README and paper
│   ├── vehicle_count.png
│   ├── congestion_score.png
│
├── main.py                    # Full pipeline script
├── tracker.py                 # Kalman filter tracker implementation
...
This project uses the Ultralytics YOLOv8 package, NumPy, OpenCV, and Matplotlib.
...
Output CSVs for vehicle_count.csv and congestion_score.csv
...
YOLOv8 performs per-frame detection of vehicles. Detections include bounding boxes, class labels, and confidence values. Only vehicle classes are retained.
...
IoU-based assignment is used to associate detections with existing tracks. Tracks are created, updated, or deleted based on assignment results, producing stable trajectory estimates.

Traffic Density

Vehicle density at frame t is computed as:

N_t = number of tracked vehicles

Congestion Score

Congestion is normalized relative to the maximum density:

C_t = N_t / max_k(N_k)


This produces a stable congestion metric in the interval [0,1].

Results Summary

The system accurately tracks vehicles in a real highway video.

Density curves show natural fluctuations in traffic flow.

The congestion metric identifies peak and low-load intervals.

YOLOv8 detections combined with Kalman filtering produce stable tracking without double-counting.
